package cn.ncbh.ncw.ncwjavafx.network.client;
...
public class IPv6Client extends Thread {
    ...
    public void run() {
        try {
            ...
            logger.log(LEVEL.INFO, "IPv6Client", "Connected to server: " + this.IPv6Address + ", port: " + this.Port);
            threadLogger.log(ThreadLogger.LogLevel.INFO, "Connected to server: " + this.IPv6Address + ", port: " + this.Port, "IPv6Client", null);
            OutputStream outToServer = client.getOutputStream();
            DataOutputStream out = new DataOutputStream(outToServer);
            out.writeUTF(System.currentTimeMillis() + this.Message);
            DataInputStream in = new DataInputStream(client.getInputStream());
            String response = in.readUTF();
            logger.log(LEVEL.INFO, "IPv6Client", response);
            threadLogger.log(ThreadLogger.LogLevel.INFO, response, "IPv6Client", null);
            client.close();
        } catch (IOException e) {
            e.printStackTrace();
            logger.log(LEVEL.ERROR, "IPv6Client", "Error!" + e);
            threadLogger.log(ThreadLogger.LogLevel.ERROR, "Error!", "IPv6Client", e);
        }
    }
